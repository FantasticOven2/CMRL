...
class SimEnvSim(Env):
    ...
    def __init__(self, time_step = 0.1, eps = 0.5):
        ...
        self.goal = np.array([20.0, 1.0])

        self.next_manifold = False

    def check_constraints(self):
        on_second_manifold = (self.state[1] != 0.1 * self.state[0] - 1.0)
    ...
    def step(self, action):
        ### Projection ###
        if self.next_manifold:
            x_act = max(action[0], action[1])
            y_act = 0.1 * x_act
            _action = np.array([x_act, y_act])
